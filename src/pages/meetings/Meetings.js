                ...
                <CardContent>
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', marginBottom: '1rem' }}>
                        ...
                        <h2 style={{
                            ...
                            {meeting.meetingState === 'pending' ? 'Pending' 
                                : meeting.meetingState === 'accepted' ? 'Accepted' 
                                : 'Rejected'}
                        </h2>
                    </Box>
                    {/* partner name */}
                    <Box sx={{ display: 'flex', alignItems: 'center', marginBottom: '1rem' }}>
                        ...
                        <p style={{ fontSize: '1.3rem' }}>
                            <strong>{userData.role === 'parent' ? 'Nanny: ' : 'Parent: '}</strong> 
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', marginBottom: '1rem' }}>
                        ...
                        <p style={{ fontSize: '1.3rem' }}><strong>Date: </strong> {meeting.dateTime.day} {months[meeting.dateTime.month]} {meeting.dateTime.year}</p>
                    </Box>
                    <Box sx={{ display: 'flex', alignItems: 'center', marginBottom: '1rem' }}>
                        <AccessTimeFilled style={{ fontSize: '2rem', marginRight: '0.5rem' }} />
                        <p style={{ fontSize: '1.3rem' }}><strong>Time: </strong> {meeting.dateTime.hour}:{meeting.dateTime.minute}</p>
                    </Box>
                    {/* Type of meeting */}
                    <Box sx={{ display: 'flex', alignItems: 'center', marginBottom: '1rem' }}>
                        <PlaceRounded style={{ fontSize: '2rem', marginRight: '0.5rem' }} />
                        <p style={{ fontSize: '1.3rem' }}><strong>Meeting Type: </strong> {meeting.meetingType === 'in-person' ? 'In-person' : 'Online'}</p>
                    </Box>
                </CardContent>
            </CardActionArea>
        </Card>
...
function Meetings() {
    ...
    return (
        ...
        {userData && (
            <>
                <PageTitle title="CareNest - Meetings" />
                <Breadcrumbs />
                <h1 style={{ marginLeft: '1rem' }}>Introduction Meetings</h1>
                <p style={{ fontSize: '1.2rem', maxWidth: '1150px', alignSelf: 'center', textAlign: 'center', marginTop: '1rem' }}>
                Manage your introduction meetings!
                Use the filters to easily find meetings based on their status or date.
                You can track the current status of each meeting and view their details.
                </p>
                <Box sx={{
                    display: 'flex',
