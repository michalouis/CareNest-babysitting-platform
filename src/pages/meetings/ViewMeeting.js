        ...
        const fetchMeetingData = async () => {
            try {
                ...
                if (meetingDoc.exists()) {
                    ...
                    // Fetch nanny data
                    const nannyDoc = await getDoc(doc(FIREBASE_DB, 'users', data.nannyId));
                    if (nannyDoc.exists()) {
                        setNannyData(nannyDoc.data());
                    } else {
                        console.error('No such nanny:', data.nannyId);
                    }
                } else {
            ...
            <Box sx={{
                ...
                <Box sx={{
                    ...
                    {meetingData.meetingType == 'online' ? (
                        <Button variant="contained" startIcon={<VideocamIcon />} sx={{ backgroundColor: 'var(--clr-blue)', padding: '0.5rem 1rem' }}>
                            Σύνδεση στην Συνάντηση
                        </Button>
                    ) : (
                        <Button variant="contained" startIcon={<NavigationIcon />} sx={{ backgroundColor: 'var(--clr-blue)', padding: '0.5rem 1rem' }}>
                ...
                <Box sx={{
                    ...
                    <Button variant="contained" startIcon={<EventIcon />} sx={{ backgroundColor: 'var(--clr-blue)', padding: '0.5rem 1rem' }}>
